...
\end{enumerate}
\vspace{8pt}
\textbf{Weighted Directed Graphs} \\
\textbf{Definitions}
\begin{itemize}
    \item $s$: Source vertex; $R$: Correct computed distances; $dist(u, v)$: Distance; $w(u, v)$: Edge cost; $V$: All vertices; $D_v = \{\forall u \in R : d(u) + w(u, v)\}$; $d(v) = \min(D_v)$.
\end{itemize}
\vspace{8pt}
\textbf{Algorithm}
\begin{enumerate}
    \item Base case: $dist(s,s) = 0, R = \{s\}$
    \item While $R \neq V$: Select $v \notin R$ minimising $D_v$, update $d(v) = \min(D_v)$, add $v$ to $R$.
\end{enumerate}
\vspace{8pt}
\textbf{Loop invariant}: $\forall u \in R, dist(s, v) = \min(D_v)$
\vspace{8pt} \\
\textbf{Proof of correctness}
\begin{enumerate}
    \item Initialisation: $dist(s, s) = d(s) = 0$
    \item Termination: All vertices in $R$, representing shortest paths.
    \item Maintenance: Shortest path $P$ from $s$ to $v$. Claim: Every vertex in $P$ is in $R$ except $v$. If false, there's $x$ before $v$ not in $R$. But then $\min(D_v) \geq dist(s, v) > min(D_x)$, so $x$ would've been picked first. Contradiction.
\end{enumerate}
\vspace{8pt}
